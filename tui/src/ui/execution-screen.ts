...
export function showExecutionScreen(results: RunResult[], rawOutput: string): Promise<void> {
  return new Promise((resolve) => {
    const screen = blessed.screen({
      smartCSR: true,
      title: 'Binary Search Execution Results',
    });
    ...
    const header = blessed.box({
      ...
      height: 1,
      align: 'center',
      content: '{bold}Binary Search Execution Results{/bold}',
      tags: true,
    });
    ...
    const tableHeader = blessed.box({
      ...
      width: '90%',
      height: 1,
      content: '{bold}{cyan-fg}Target     | Status  | Index    | CPU Time(s)    | Memory(KB) | Exec Time(s){/cyan-fg}{/bold}',
      tags: true,
    });
    ...
    const resultsBox = blessed.box({
      ...
      content: results.map(r => {
        ...
        return `           ${String(r.target).padEnd(10)} | ${status.padEnd(7)} | ${index.padEnd(8)} | ${r.cpuTime.toExponential(2).padEnd(14)} | ${String(r.memoryUsage).padEnd(10)} | ${r.execTime.toExponential(2)}`;
      }).join('\n'),
    });
    ...
    const summary = blessed.box({
      top: results.length + 6,
      left: '5%',
      width: '90%',
      height: 6,
      tags: true,
      content: `{bold}Summary:{/bold}
  Total Searches: {yellow-fg}${results.length}{/yellow-fg}
  Found: {green-fg}${foundCount}{/green-fg} | Not Found: {red-fg}${results.length - foundCount}{/red-fg}
